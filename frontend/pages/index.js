    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <header className="border-b border-slate-800 bg-slate-900/70 backdrop-blur">
        <div className="mx-auto flex max-w-6xl flex-col gap-2 px-6 py-6 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-3xl font-semibold">TripSmith AI 🌍</h1>
      ...
      <main className="mx-auto flex max-w-6xl flex-col gap-10 px-6 py-10">
        <section className="grid gap-8 lg:grid-cols-[1.1fr,1fr]">
          <form
            ...
            className="grid gap-4 rounded-2xl border border-slate-800 bg-slate-900/60 p-6 shadow-xl shadow-sky-950/10"
          >
            <div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="flex flex-col gap-1 text-sm">
                ...
                <input
                  ...
                  className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 outline-none focus:border-sky-500"
                  required
                />
              </label>
              ...
              <label className="flex flex-col gap-1 text-sm">
                ...
                <input
                  ...
                  placeholder="EWR"
                  className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 uppercase outline-none focus:border-sky-500"
                />
              </label>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="flex flex-col gap-1 text-sm">
                <span className="font-semibold text-slate-200">Start Date</span>
                <input
                  type="date"
                  value={startDate}
              ...
              <label className="flex flex-col gap-1 text-sm">
                <span className="font-semibold text-slate-200">End Date</span>
                <input
                  type="date"
                  value={endDate}
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <label className="flex flex-col gap-1 text-sm">
                <span className="font-semibold text-slate-200">Budget (USD)</span>
                <input
                  type="number"
                  min="0"
                  value={budget}
              ...
              <label className="flex flex-col gap-1 text-sm">
                <span className="font-semibold text-slate-200">Travelers</span>
                <input
                  type="number"
                  min="1"
                  value={travelers}
              ...
              <label className="flex flex-col gap-1 text-sm">
                <span className="font-semibold text-slate-200">Contact Email</span>
                <input
                  type="email"
                  value={email}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="flex flex-col gap-1 text-sm">
                <span className="font-semibold text-slate-200">What do you love?</span>
                <input
                  type="text"
                  value={likes}
              ...
              <label className="flex flex-col gap-1 text-sm">
                <span className="font-semibold text-slate-200">Anything to avoid?</span>
                <input
                  type="text"
                  value={dislikes}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="flex flex-col gap-1 text-sm">
                <span className="font-semibold text-slate-200">Travel Style</span>
                <select
                  value={travelStyle}
              ...
              <label className="flex items-center gap-2 text-sm">
                <input
                  ...
                  className="h-4 w-4 rounded border-slate-700 bg-slate-950 text-sky-500 focus:ring-0"
                />
                <span className="font-semibold text-slate-200">
            ...
            <button
              type="submit"
              disabled={loading}
              className="mt-2 inline-flex items-center justify-center rounded-lg bg-sky-500 px-4 py-3 text-sm font-semibold text-white transition hover:bg-sky-400 disabled:cursor-wait disabled:bg-slate-700"
            >
              {loading ? "Planning your trip…" : "Generate Smart Plan"}
            </button>

            {error && (
              <p className="rounded-lg border border-red-400 bg-red-500/10 px-3 py-2 text-sm text-red-200">
                {error}
              </p>
            )}
          </form>
        ...
        {plan && (
          <section className="grid gap-8">
            <div className="grid gap-4 rounded-2xl border border-slate-800 bg-slate-900/60 p-6 md:grid-cols-2">
              <div>
                <h3 className="text-xl font-semibold text-white">Trip Snapshot</h3>
                <p className="text-sm text-slate-300">
                  {plan.destination} • {plan.travelers} traveler(s) •{" "}
                  {plan.start_date} → {plan.end_date}
                </p>
              </div>
              <div className="grid gap-2 text-sm text-slate-200 md:text-right">
                <p>
                  Budget: <span className="font-semibold">{currency.format(plan.budget)}</span>
                </p>
                <p>
                  Sustainability tier:{" "}
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
                <h3 className="text-lg font-semibold text-white">Flight Options</h3>
                <div className="mt-4 space-y-4">
                  {plan.flights?.map((flight) => (
                    <label
                      key={flight.id}
                      className="block rounded-xl border border-slate-800 bg-slate-950/70 p-4 hover:border-sky-500"
                    >
              ...
              <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
                <h3 className="text-lg font-semibold text-white">Lodging Suggestions</h3>
                <div className="mt-4 space-y-4">
                      ...
                      <div className="flex items-center justify-between text-sm">
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            className="h-4 w-4 text-sky-500 focus:ring-0"
                          />
                          <span className="font-semibold text-slate-100">{stay.name}</span>
                        </div>
                        <span className="font-semibold text-sky-300">
                          {currency.format(stay.nightly_rate)}/night
                        </span>
                      </div>
                      <p className="mt-1 text-xs text-slate-300">{stay.address}</p>
            ...
            <div className="grid gap-6 lg:grid-cols-2">
                      ...
                      <ul className="mt-3 space-y-2 text-xs text-slate-200">
                        {day.activities?.map((activity, idx) => (
                          <li key={`${day.date}-activity-${idx}`}>
                            <span className="font-semibold text-sky-300">{activity.time}</span>{" "}
              ...
              <div className="grid gap-6">
                <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
                  <h3 className="text-lg font-semibold text-white">Weather Outlook</h3>
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
                <h3 className="text-lg font-semibold text-white">Must-See Spots</h3>
                <ul className="mt-3 space-y-2 text-sm text-slate-200">
                  {plan.points_of_interest?.map((poi, idx) => (
                    <li key={`poi-${idx}`} className="rounded-lg border border-slate-800 bg-slate-950/60 px-3 py-2">
                      <span className="font-semibold text-sky-300">{poi.name}</span>{" "}
                      <span className="text-xs uppercase text-slate-400">
                        {poi.category?.split(",")[0]}
                      </span>
                      {poi.description && (
                        <p className="text-xs text-slate-300">{poi.description}</p>
                      )}
                    </li>
                  ))}
                </ul>
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
              ...
              {bookingStatus && (
                <p className="mt-3 text-sm text-slate-200">{bookingStatus}</p>
              )}
              {booking && (
                <div className="mt-4 rounded-xl border border-emerald-500/50 bg-emerald-500/10 p-4 text-sm text-emerald-200">
